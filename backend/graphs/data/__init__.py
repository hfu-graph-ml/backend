...
for sample in random.sample(all_nodes, k=config['data']['num_nodes']):
  base_graph.add_node(
      sample[0],
      age=feature_list_age[sample[2]],
      country=feature_list_country[sample[3]],
      drinker=feature_list_drinker[sample[4]],
      relationship=feature_list_relationship[sample[5]] if feature_list_relationship[sample[5]] != None else len(feature_list_relationship) + sample[0]
  )
